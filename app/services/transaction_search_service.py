...
class TransactionSearchService:
    ...
    def _transform_transaction(
        ...
        match_status_label = TransactionSearchService._get_match_status_label(
    ...
    def _group_transactions_by_recon_ref(
        ...
        grouped: Dict[str, Dict[str, List[TransactionDetailItem]]] = defaultdict(
        ...
        for row in transactions:
            ...
            source_key = (
                f"{source_name.lower()}_transactions"
                if source_name
                else "unknown_transactions"
            )
    ...
    async def smart_search(
        search_params: SmartSearchRequest, db: AsyncSession
    ) -> TransactionSearchResponse:
        """
        ...
        if search_params.search_query:
            categorized = SmartSearchDetector.parse_search_query(
            ...
            if categorized["account_numbers"]:
                for account in categorized["account_numbers"]:
                    or_conditions.append(
                        Transaction.account_number.ilike(f"%{account}%")
                    )
        ...
        total_pages = (
            math.ceil(total_groups / search_params.page_size) if total_groups > 0 else 0
        )
        offset = (search_params.page - 1) * search_params.page_size

        # Get distinct recon_reference_numbers with pagination
        recon_refs_query = select(
            func.distinct(Transaction.recon_reference_number)
        ).select_from(Transaction)
        if filters:
            recon_refs_query = recon_refs_query.where(*filters)

        recon_refs_query = (
            recon_refs_query.order_by(Transaction.recon_reference_number.desc())
            .offset(offset)
        ...
        if paginated_recon_refs:
            transactions_query = (
                ...
                .outerjoin(
                    MatchingRuleConfig,
                    Transaction.match_rule_id == MatchingRuleConfig.id,
                )
                .where(Transaction.recon_reference_number.in_(paginated_recon_refs))
                .order_by(
                    Transaction.recon_reference_number.desc(), Transaction.source_id
                )
            )
            ...
            grouped_transactions = (
                TransactionSearchService._group_transactions_by_recon_ref(
                    all_transactions
                )
            )
            ...
            summary = TransactionSearchService._calculate_summary(
        ...
        return TransactionSearchResponse(
            transactions=grouped_transactions, pagination=pagination, summary=summary
        )

    @staticmethod
    async def search_transactions(
        search_params: TransactionSearchRequest, db: AsyncSession
    ) -> TransactionSearchResponse:
        """
        ...
        if search_params.reference_number:
            filters.append(
                Transaction.reference_number.ilike(
                    f"%{search_params.reference_number}%"
                )
            )
