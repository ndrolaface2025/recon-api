...
class ApplicationMatcher:
    ...
    async def execute_complex_matching(
        ...
        try:
            ...
            # Determine match type (FULL or PARTIAL)
            match_type = (
                "PARTIAL" if min_sources and min_sources < len(sources) else "FULL"
            )
            ...
            unmatched_count = await self._update_unmatched_transactions(
                transactions_by_source, all_matched_txn_ids, rule_id, recon_group_number
            )
    ...
    async def _fetch_transactions_by_sources(
        self, sources: List[str], channel_id: Optional[int]
    ) -> Dict[str, List[Dict[str, Any]]]:
        """
        Fetch unmatched transactions for all sources
        ...
        for source_name in sources:
            ...
            transactions[source_name] = [
                {
                    ...
                    # Format: CARD|TERMINAL|MERCHANT
                    **(
                        self._parse_other_details(row.otherDetails)
                        if row.otherDetails
                        else {}
                    ),
                }
            ...
            logger.info(
                f"Fetched {len(transactions[source_name])} transactions from {source_name}"
            )
    ...
    def _find_matching_groups(
        ...
        if isinstance(condition_groups, str):
            # Replace single = with == but avoid replacing == with ====
            condition_expr = condition_groups.replace(
                "==", "@@DOUBLE_EQ@@"
            )  # Protect existing ==
            condition_expr = condition_expr.replace("=", "==")  # Convert single = to ==
        ...
        logger.info(
            f"🔍 Finding matches: min_sources={min_sources}, total_sources={len(source_names)}, sources={source_names}"
        )
        ...
        logic_expr_str = (
            ast.unparse(tree) if hasattr(ast, "unparse") else condition_expr
        )
        ...
        if not matching_fields:
            logger.warning(
                "⚠️  No equality fields found in condition, defaulting to reference_number only"
            )
            matching_fields.add("reference_number")

        logger.info(
            f"   Grouping transactions by composite key with fields: {sorted(matching_fields)}"
        )
        ...
        for source_name in source_names:
            ...
            for txn in source_txns:
                ...
                if not key_parts:
                    logger.debug(
                        f"   Skipping transaction {txn.get('id')} - no matching field values"
                    )
                    continue
        ...
        logger.info(
            f"   Found {len(txns_by_key)} unique matching keys (RRN + other fields)"
        )
        ...
        for composite_key, sources_dict in txns_by_key.items():
            ...
            if num_sources < min_sources:
                logger.warning(
                    f"   Key {composite_key}: SKIPPED - only {num_sources} sources (min required: {min_sources}), sources={available_sources}"
                )
                continue

            logger.info(
                f"   Key {composite_key}: {num_sources} sources available ({available_sources}), min_sources={min_sources}, total_sources={len(source_names)}"
            )
            ...
            if num_sources == len(source_names):
                ...
                for txn_tuple in product(*source_lists):
                    ...
                    try:
                        if eval(compiled_expr, {}, context):
                            ...
                            rrn_value = (
                                composite_key.split("|")[0]
                                if "|" in composite_key
                                else composite_key
                            )
                            logger.info(
                                f"✅ Match Found (FULL) --> Key={composite_key}, IDs={[context[src].id for src in source_names]}"
                            )
                            matched_groups.append(
                                {
                                    "transactions": [txn for txn in txn_tuple],
                        ...
                        logger.debug(
                            f"Condition evaluation error for key {composite_key}: {e}"
                        )
                        continue
                ...
                # Partial match - we have min_sources <= num_sources < total_sources
                logger.info(
                    f"   🔄 Key {composite_key}: Attempting PARTIAL match with {num_sources} sources (need {min_sources}, total possible {len(source_names)})"
                )
                # Try all combinations of available sources that meet min_sources
                for combo_size in range(num_sources, min_sources - 1, -1):
                    logger.debug(f"      Trying combinations of size {combo_size}")
                    for source_combo in combinations(available_sources, combo_size):
                        ...
                        for txn_tuple in product(*source_lists):
                            ...
                            try:
                                ...
                                if eval(partial_expr, {}, context):
                                    logger.warning(
                                        f"✅ Match Found (PARTIAL) --> Key={composite_key}, sources={source_combo}, IDs={[context[src].id for src in source_combo]}"
                                    )
                                    matched_groups.append(
                                        {
                                            "transactions": [txn for txn in txn_tuple],
                                    ...
                                    logger.debug(
                                        f"         Expression evaluated to False"
                                    )
                            except Exception as e:
                                logger.warning(
                                    f"Partial match evaluation error for key {composite_key}: {e}"
                                )
                                continue
                        else:
                            continue
        ...
        for group in condition_groups:
            ...
            for condition in conditions:
                ...
                if "group_type" in condition:
                    # Recursive evaluation
                    result = self._evaluate_condition_groups(
                        transaction_group, [condition], tolerance
                    )
                    results.append(result)
                else:
                    # Evaluate individual condition
    ...
    def _check_within_tolerance(
        self, values: List[Any], field: str, tolerance: Optional[Dict[str, Any]]
    ) -> bool:
        """Check if all values are within tolerance"""
        if not values or None in values:
            return False
    ...
    def _check_comparison(
        self, values: List[Any], condition: Dict[str, Any], comparison_type: str
    ) -> bool:
        """
        Check comparison operators: >, <, >=, <=
    ...
    def _check_string_operation(
        self, values: List[Any], condition: Dict[str, Any], operation: str
    ) -> bool:
        """
        Check string operations: starts_with, ends_with, contains, regex
    ...
    def _check_null_handling(self, values: List[Any], should_be_null: bool) -> bool:
        """
        Check NULL handling: is_null, is_not_null
        ...
        if should_be_null:
            ...
            # All values should NOT be None/null
            return all(
                v is not None and v != "" and str(v).lower() != "none" for v in values
            )

    def _check_cross_field(
        self, transaction_group: Dict[str, Dict[str, Any]], condition: Dict[str, Any]
    ) -> bool:
        """
        Check cross-field conditions: Compare different fields
        ...
        if not field1 or not field2:
            logger.warning(
                "cross_field_equals requires 'field1' and 'field2' parameters"
            )
            return False
    ...
    async def _update_matched_transactions(
        ...
        """
        Update all matched transactions in database
        Set match_status, reconciliation_status, matched_with_txn_id
    ...
    async def _update_unmatched_transactions(
        ...
        logger.info(
            f"📝 Updating {len(unmatched_txn_ids)} unmatched transactions with recon_group_number"
        )
        ...
        await self.db.execute(
            text(update_query),
            {"recon_group_number": recon_group_number, "txn_ids": unmatched_txn_ids},
        )
