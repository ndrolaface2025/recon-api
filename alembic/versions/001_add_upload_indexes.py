...
def upgrade():
    ...
    op.create_index("idx_txn_date", "tbl_txn_transactions", ["date"], unique=False)
    ...
    op.create_index(
        "idx_upload_file_status", "tbl_upload_files", ["status"], unique=False
    )

    # Index for upload file channel
    op.create_index(
        "idx_upload_file_channel", "tbl_upload_files", ["channel_id"], unique=False
    )
