...
#         )
#         .where(Transaction.recon_group_number == recon_group_number)
#         .where(Transaction.reconciled_mode == reconciled_mode)
    ...
    if mode == "AUTOMATIC":
        ...
        base_stmt = (
            select(
                ...
                case(
                    (ManualTransaction.reconciled_status.is_(True), 1),
                    else_=0,
                ).label("match_status"),
                literal(2).label("reconciled_mode"),  # ✅ FIX HERE
                ManualTransaction.created_at,
                ManualTransaction.comment,
                ManualTransaction.reconciled_by,
            )
            .where(ManualTransaction.manual_txn_id.isnot(None))
            .where(
                ManualTransaction.manual_txn_id.in_(
                    select(Transaction.txn_id).where(
                        Transaction.recon_group_number == recon_group_number
                    )
                )
            )
