    ...
    async def execute_complex_matching(
        ...
        try:
            ...
            all_txn_ids = await self._update_matched_transactions(
                matched_groups, rule_id, match_type, total_sources=len(sources)
            )
    ...
    def _find_matching_groups(
        ...
        for primary_txn in transactions_by_source[primary_source]:
            ...
            if len(candidates) >= min_sources:
                ...
                if self._evaluate_condition_groups(
                    candidates, condition_groups, tolerance
                ):
                    logger.warning(
                    ...
                    matched_groups.append(
                        {
                            ...
                            "match_key": primary_txn.get(
                                "rrn", f"group_{len(matched_groups)}"
                            ),
