    ...
    try:
        ...
        query = (
            select(
                ...
                func.count(case((Transaction.match_status == 1, 1))).label(
                    "matched_count"
                ),
                func.count(case((Transaction.match_status == 2, 1))).label(
                    "partial_count"
                ),
                func.count(
                    case(
                ...
                func.sum(
                    case(
                        (
                            ...
                            func.cast(Transaction.amount, Numeric),
                        )
                    )
            ...
            .outerjoin(Transaction, ChannelConfig.id == Transaction.channel_id)
            .group_by(ChannelConfig.id, ChannelConfig.channel_name)
        ...
        for row in rows:
            ...
            unmatched_pct = (
                (unmatched_count / total_count * 100) if total_count > 0 else 0
            )
            ...
            channels.append(
                {
                    ...
                    "total": {"count": total_count, "amount": f"{total_amount:.2f}"},
                }
            )
        ...
        overall_partial_pct = (
            (overall_partial / overall_total * 100) if overall_total > 0 else 0
        )
        overall_unmatched_pct = (
            (overall_unmatched / overall_total * 100) if overall_total > 0 else 0
        )
...
async def get_transaction_counts(
    ...
    try:
        ...
        full_match_query = select(
            func.count(func.distinct(Transaction.reference_number)).label(
                "matched_groups"
            )
        ).where(
            and_(Transaction.channel_id == channel_id, Transaction.match_status == 1)
        )
        ...
        partial_match_query = select(
            ...
            and_(Transaction.channel_id == channel_id, Transaction.match_status == 2)
        )
        ...
        return {
            ...
            "data": {"matched": 0, "partially_matched": 0, "not_matched": 0},
        }
...
async def get_transaction_details(
    ...
    try:
        ...
        if txn.reference_number and txn.match_status in [1, 2]:
            matched_query = (
                select(Transaction, SourceConfig.source_name)
                .outerjoin(SourceConfig, Transaction.source_id == SourceConfig.id)
                .where(
                    and_(
                        ...
                        Transaction.id
                        != transaction_id,  # Exclude the main transaction
                    )
                )
        ...
        return {
            ...
            "data": {
                ...
                "match_group_id": (
                    f"{txn.reference_number}-{txn.channel_id}"
